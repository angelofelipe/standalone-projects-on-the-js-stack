...
app.use(express.urlencoded({extendable: true}))
...
function authenticate(req, res, next) {
   ...
   try {
      ...
      jwt.verify(cookie.refresh_t, process.env.REFRESH_TOKEN_KEY, (err, user) => {
         ...
         const token = jwt.sign({ username: user.username }, process.env.ACESS_TOKEN_KEY, { expiresIn: 10 });
...
app.post("/login", (req, res) => {
    ...
    console.log(db)
    const result = db.find((u) => u.username === user.username);
    if (!result) {
   ...
   console.log(process.env.ACESS_TOKEN_KEY)
   console.log(process.env.REFRESH_TOKEN_KEY)
    ...
    res.clearCookie();
    res.cookie("acess_t", cookie.acess_t);
    res.cookie("refresh_t", cookie.refresh_t);
    return res.status(200).send({ message: "Usuário Logado com sucesso", token: cookie.acess_t, refresh_t: cookie.refresh_t});
 });
